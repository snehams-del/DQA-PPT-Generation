...
async def run_evaluation_for_task(domain: str, task, adk_agent, user_llm: str, run_path: Path):
    ...
    for turn in range(15):
        ...
        if adk_tool_call:
            ...
            task_logger.log(f"  [ADK AGENT -> Harness]: Tool Call: {adk_tool_name}({adk_args})")
    ...
    dummy_sim_run = SimulationRun(
        id=f"{run_path.name}_task_{task.id}", task_id=task.id, start_time="", end_time="", duration=0,
...
def write_summary_files(run_path: Path, all_task_results: list, args, adk_agent):
    ...
    with open(csv_path, 'w', newline='', encoding='utf-8') as f:
        ...
        for result in all_task_results:
            writer.writerow({'task_id': result['task_id'], 'reward': result['reward'], 'passed': result['passed']})
...
async def main(args):
    ...
    task_coroutines = [run_evaluation_for_task(args.domain, task, adk_agent, args.user_llm, run_path) for task in tasks]
    all_task_results = []
    ...
    with open(os.devnull, 'w') as devnull:
        with redirect_stderr(devnull):
            for future in tqdm.as_completed(task_coroutines, total=len(tasks), desc=f"Evaluating {args.domain} domain", file=sys.stdout):
                try:
                    result = await future
...
if __name__ == "__main__":
    ...
    parser.add_argument("--adk_agent_path", type=str, required=True, help="Path to ADK agent. e.g., 'sample_adk_agents/airline/agent.py:variable_name'")
