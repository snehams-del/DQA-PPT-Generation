...
from src.vars.gvaccountusage import AccountUsageCopyHistory

class CopyHistory:
    def __init__(self, session):
        self.col = AccountUsageCopyHistory().columns
        self.session = session

    def get_copy_history_for_table(self, table_catalog_name, table_schema_name, table_name):
        df = self.session.table(self.col._view).filter(
            (col(self.col._table_catalog_name) == table_catalog_name.upper()) &
            (col(self.col._table_schema_name) == table_schema_name.upper()) &
            (col(self.col._table_name) == table_name.upper())
        ).collect()
        return df

    def get_failed_copies(self):
        df = self.session.table(self.col._view).filter(
            col(self.col._status) == "LOAD_FAILED"
        ).collect()
        return df

    def get_copy_history_in_time_range(self, start_time, end_time):
        df = self.session.table(self.col._view).filter(
    ...
    def get_copy_history_by_pipe(self, pipe_catalog_name, pipe_schema_name, pipe_name):
        df = self.session.table(self.col._view).filter(
    ...
    def get_copy_errors_for_table(self, table_catalog_name, table_schema_name, table_name):
        df = self.session.table(self.col._view).filter(
            (col(self.col._table_catalog_name) == table_catalog_name.upper()) &
            (col(self.col._table_schema_name) == table_schema_name.upper()) &
            (col(self.col._table_name) == table_name.upper()) &
