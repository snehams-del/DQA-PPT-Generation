...
logger.debug("[SESSION_POLICY] Loading skill: snowflake-create-session-policy")
_skill_toolset = SkillToolset(
    skills=[load_skill_from_dir(_skills_dir / "snowflake-create-session-policy")]
)
logger.debug("[SESSION_POLICY] Skill loaded successfully")


ag_sf_manage_session_policy = LlmAgent(
