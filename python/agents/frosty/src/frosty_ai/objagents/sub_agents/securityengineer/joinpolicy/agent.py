...
logger.debug("[JOIN_POLICY] Loading skill: snowflake-create-join-policy")
_skill_toolset = SkillToolset(
    skills=[load_skill_from_dir(_skills_dir / "snowflake-create-join-policy")]
)
logger.debug("[JOIN_POLICY] Skill loaded successfully")


ag_sf_manage_join_policy = LlmAgent(
