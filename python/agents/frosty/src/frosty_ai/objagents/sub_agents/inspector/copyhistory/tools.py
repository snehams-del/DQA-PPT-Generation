from src.infschema.copyhistory import CopyHistory
...
def get_copy_history(database_name: str, table_name: str, tool_context: ToolContext) -> dict:
    """
    Get copy history data.
    ...
    Returns:
        Dictionary with copy history records
    """
    logger = logging.getLogger(tool_context.state.get("app:LOGGER")).getChild(__name__)
    logger.debug("get_copy_history called for database '%s'", database_name)
    ...
    try:
        ...
        return {
            ...
            "records": records,
            "count": len(records),
            "message": f"Found {len(records)} copy history record(s)"
        }
    except SnowparkSQLException as e:
        logger.error("Snowflake SQL error in get_copy_history: %s", str(e))
        return {"records": [], "count": None, "error": str(e), "message": f"Snowflake SQL error: {str(e)}"}
    except Exception as e:
        logger.error("Error in get_copy_history: %s", str(e))
