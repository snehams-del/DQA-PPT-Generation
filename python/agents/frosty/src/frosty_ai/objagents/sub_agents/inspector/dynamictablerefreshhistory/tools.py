from src.infschema.dynamictablerefreshhistory import DynamicTableRefreshHistory
...
def get_dynamic_table_refresh_history(database_name: str, tool_context: ToolContext, result_limit: int = 100) -> dict:
    """
    Get dynamic table refresh history data.
    ...
    Returns:
        Dictionary with dynamic table refresh history records
    """
    logger = logging.getLogger(tool_context.state.get("app:LOGGER")).getChild(__name__)
    logger.debug("get_dynamic_table_refresh_history called for database '%s'", database_name)
    session = _get_session(tool_context)
    try:
        inspector = DynamicTableRefreshHistory(session)
        ...
        return {
            ...
            "records": records,
            "count": len(records),
            "message": f"Found {len(records)} dynamic table refresh history record(s)"
        }
    except SnowparkSQLException as e:
        logger.error("Snowflake SQL error in get_dynamic_table_refresh_history: %s", str(e))
        return {"records": [], "count": None, "error": str(e), "message": f"Snowflake SQL error: {str(e)}"}
    except Exception as e:
        logger.error("Error in get_dynamic_table_refresh_history: %s", str(e))
