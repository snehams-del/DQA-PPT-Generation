from src.infschema.applicationconfigurationvaluehistory import ApplicationConfigurationValueHistory
...
def get_application_configuration_value_history(database_name: str, tool_context: ToolContext, result_limit: int = 100) -> dict:
    """
    Get application configuration value history data.
    ...
    Returns:
        Dictionary with application configuration value history records
    """
    logger = logging.getLogger(tool_context.state.get("app:LOGGER")).getChild(__name__)
    logger.debug("get_application_configuration_value_history called for database '%s'", database_name)
    session = _get_session(tool_context)
    try:
        inspector = ApplicationConfigurationValueHistory(session)
        ...
        return {
            ...
            "records": records,
            "count": len(records),
            "message": f"Found {len(records)} application configuration value history record(s)"
        }
    except SnowparkSQLException as e:
        logger.error("Snowflake SQL error in get_application_configuration_value_history: %s", str(e))
        return {"records": [], "count": None, "error": str(e), "message": f"Snowflake SQL error: {str(e)}"}
    except Exception as e:
        logger.error("Error in get_application_configuration_value_history: %s", str(e))
