from src.infschema.icebergtablefiles import IcebergTableFiles
...
def get_iceberg_table_files(database_name: str, table_name: str, tool_context: ToolContext) -> dict:
    """
    Get iceberg table files data.
    ...
    Returns:
        Dictionary with iceberg table files records
    """
    logger = logging.getLogger(tool_context.state.get("app:LOGGER")).getChild(__name__)
    logger.debug("get_iceberg_table_files called for database '%s'", database_name)
    session = _get_session(tool_context)
    try:
        inspector = IcebergTableFiles(session)
        df = inspector.get_history(database_name, table_name)
        records = [row.as_dict() for row in df]
        return {
            "database_name": database_name.upper(),
            "records": records,
            "count": len(records),
            "message": f"Found {len(records)} iceberg table files record(s)"
        }
    except SnowparkSQLException as e:
        logger.error("Snowflake SQL error in get_iceberg_table_files: %s", str(e))
        return {"records": [], "count": None, "error": str(e), "message": f"Snowflake SQL error: {str(e)}"}
    except Exception as e:
        logger.error("Error in get_iceberg_table_files: %s", str(e))
