from src.infschema.warehouseloadhistory import WarehouseLoadHistory
...
def get_warehouse_load_history(database_name: str, tool_context: ToolContext, warehouse_name: str = None, result_limit: int = 100) -> dict:
    """
    Get warehouse load history data.

    Args:
        database_name: Name of the database
        warehouse_name: Optional name of the warehouse
        result_limit: Maximum number of records to return
        tool_context: ADK tool context

    Returns:
        Dictionary with warehouse load history records
    """
    logger = logging.getLogger(tool_context.state.get("app:LOGGER")).getChild(__name__)
    logger.debug("get_warehouse_load_history called for database '%s'", database_name)
    session = _get_session(tool_context)
    try:
        inspector = WarehouseLoadHistory(session)
        df = inspector.get_history(database_name, warehouse_name, result_limit)
        records = [row.as_dict() for row in df]
        return {
            "database_name": database_name.upper(),
            "records": records,
            "count": len(records),
            "message": f"Found {len(records)} warehouse load history record(s)"
        }
    except SnowparkSQLException as e:
        logger.error("Snowflake SQL error in get_warehouse_load_history: %s", str(e))
        return {"records": [], "count": None, "error": str(e), "message": f"Snowflake SQL error: {str(e)}"}
    except Exception as e:
        logger.error("Error in get_warehouse_load_history: %s", str(e))
