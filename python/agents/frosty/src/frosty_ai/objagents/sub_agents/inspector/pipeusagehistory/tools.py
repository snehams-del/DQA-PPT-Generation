from src.infschema.pipeusagehistory import PipeUsageHistory
...
def get_pipe_usage_history(database_name: str, tool_context: ToolContext, result_limit: int = 100) -> dict:
    """
    Get pipe usage history data.
    ...
    Returns:
        Dictionary with pipe usage history records
    """
    logger = logging.getLogger(tool_context.state.get("app:LOGGER")).getChild(__name__)
    logger.debug("get_pipe_usage_history called for database '%s'", database_name)
    session = _get_session(tool_context)
    try:
        inspector = PipeUsageHistory(session)
        ...
        return {
            ...
            "records": records,
            "count": len(records),
            "message": f"Found {len(records)} pipe usage history record(s)"
        }
    except SnowparkSQLException as e:
        logger.error("Snowflake SQL error in get_pipe_usage_history: %s", str(e))
        return {"records": [], "count": None, "error": str(e), "message": f"Snowflake SQL error: {str(e)}"}
    except Exception as e:
        logger.error("Error in get_pipe_usage_history: %s", str(e))
