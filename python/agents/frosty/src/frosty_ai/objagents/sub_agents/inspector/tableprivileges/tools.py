from src.infschema.tableprivileges import TablePrivileges
...
def list_table_privileges(database_name: str, schema_name: str, table_name: str, tool_context: ToolContext) -> dict:
    """
    List table privilege filtered by table.
    ...
    Returns:
        Dictionary with list of table privilege
    """
    logger = logging.getLogger(tool_context.state.get("app:LOGGER")).getChild(__name__)
    logger.debug("list_table_privileges called for '%s.%s.%s'", database_name, schema_name, table_name)
    session = _get_session(tool_context)
    try:
        inspector = TablePrivileges(session)
        ...
        return {
            ...
            "records": records,
            "count": len(records),
            "message": f"Found {len(records)} table privilege record(s) for table '{str(table_name).upper()}'"
        }
    except SnowparkSQLException as e:
        logger.error("Snowflake SQL error in list_table_privileges: %s", str(e))
        return {"records": [], "count": None, "error": str(e), "message": f"Snowflake SQL error: {str(e)}"}
    except Exception as e:
        logger.error("Error in list_table_privileges: %s", str(e))
        return {"records": [], "count": None, "error": str(e), "message": f"Error: {str(e)}"}

def list_privileges_by_grantee(database_name: str, schema_name: str, grantee: str, tool_context: ToolContext) -> dict:
    """
    List table privilege filtered by grantee.
    ...
    Returns:
        Dictionary with filtered table privilege
    """
    logger = logging.getLogger(tool_context.state.get("app:LOGGER")).getChild(__name__)
    logger.debug("list_privileges_by_grantee called for '%s.%s' with grantee='%s'", database_name, schema_name, grantee)
    session = _get_session(tool_context)
    try:
        inspector = TablePrivileges(session)
        inspector.use_database(database_name)
        ...
        return {
            ...
            "records": records,
            "count": len(records),
            "message": f"Found {len(records)} table privilege record(s) with grantee '{str(grantee).upper()}'"
        }
    except SnowparkSQLException as e:
        logger.error("Snowflake SQL error in list_privileges_by_grantee: %s", str(e))
        return {"records": [], "count": None, "error": str(e), "message": f"Snowflake SQL error: {str(e)}"}
    except Exception as e:
        logger.error("Error in list_privileges_by_grantee: %s", str(e))
        return {"records": [], "count": None, "error": str(e), "message": f"Error: {str(e)}"}
