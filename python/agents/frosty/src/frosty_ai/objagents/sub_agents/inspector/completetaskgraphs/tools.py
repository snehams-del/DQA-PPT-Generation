from src.infschema.completetaskgraphs import CompleteTaskGraphs
...
def get_complete_task_graphs(database_name: str, tool_context: ToolContext, result_limit: int = 100) -> dict:
    """
    Get complete task graphs data.
    ...
    Returns:
        Dictionary with complete task graphs records
    """
    logger = logging.getLogger(tool_context.state.get("app:LOGGER")).getChild(__name__)
    logger.debug("get_complete_task_graphs called for database '%s'", database_name)
    session = _get_session(tool_context)
    try:
        inspector = CompleteTaskGraphs(session)
        ...
        return {
            ...
            "records": records,
            "count": len(records),
            "message": f"Found {len(records)} complete task graphs record(s)"
        }
    except SnowparkSQLException as e:
        logger.error("Snowflake SQL error in get_complete_task_graphs: %s", str(e))
        return {"records": [], "count": None, "error": str(e), "message": f"Snowflake SQL error: {str(e)}"}
    except Exception as e:
        logger.error("Error in get_complete_task_graphs: %s", str(e))
