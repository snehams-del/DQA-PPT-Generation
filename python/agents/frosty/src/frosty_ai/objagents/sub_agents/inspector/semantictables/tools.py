from src.infschema.semantictables import SemanticTables
...
def list_semantic_tables(database_name: str, schema_name: str, semantic_view_name: str, tool_context: ToolContext) -> dict:
    """
    List semantic table filtered by semantic view.
    ...
    Returns:
        Dictionary with list of semantic table
    """
    logger = logging.getLogger(tool_context.state.get("app:LOGGER")).getChild(__name__)
    logger.debug("list_semantic_tables called for '%s.%s.%s'", database_name, schema_name, semantic_view_name)
    session = _get_session(tool_context)
    try:
        inspector = SemanticTables(session)
        ...
        return {
            ...
            "records": records,
            "count": len(records),
            "message": f"Found {len(records)} semantic table record(s) for semantic view '{str(semantic_view_name).upper()}'"
        }
    except SnowparkSQLException as e:
        logger.error("Snowflake SQL error in list_semantic_tables: %s", str(e))
        return {"records": [], "count": None, "error": str(e), "message": f"Snowflake SQL error: {str(e)}"}
    except Exception as e:
        logger.error("Error in list_semantic_tables: %s", str(e))
