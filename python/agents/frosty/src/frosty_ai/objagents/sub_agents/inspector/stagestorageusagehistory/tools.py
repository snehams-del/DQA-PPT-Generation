from src.infschema.stagestorageusagehistory import StageStorageUsageHistory
...
def get_stage_storage_usage_history(database_name: str, tool_context: ToolContext, result_limit: int = 100) -> dict:
    """
    Get stage storage usage history data.
    ...
    Returns:
        Dictionary with stage storage usage history records
    """
    logger = logging.getLogger(tool_context.state.get("app:LOGGER")).getChild(__name__)
    logger.debug("get_stage_storage_usage_history called for database '%s'", database_name)
    session = _get_session(tool_context)
    try:
        inspector = StageStorageUsageHistory(session)
        ...
        return {
            ...
            "records": records,
            "count": len(records),
            "message": f"Found {len(records)} stage storage usage history record(s)"
        }
    except SnowparkSQLException as e:
        logger.error("Snowflake SQL error in get_stage_storage_usage_history: %s", str(e))
        return {"records": [], "count": None, "error": str(e), "message": f"Snowflake SQL error: {str(e)}"}
    except Exception as e:
        logger.error("Error in get_stage_storage_usage_history: %s", str(e))
