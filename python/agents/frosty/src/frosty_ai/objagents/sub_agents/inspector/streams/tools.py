...
def check_stream_exists(database_name: str, schema_name: str, stream_name: str, tool_context: ToolContext) -> dict:
    """
    Check if a stream exists in Snowflake.
    ...
    logger.debug("check_stream_exists called for '%s.%s.%s'", database_name, schema_name, stream_name)
    ...
    # Check stream existence using SHOW STREAMS command
    try:
        query = f"SHOW STREAMS LIKE '{stream_name.upper()}' IN SCHEMA {database_name}.{schema_name.upper()}"
    ...
    return {
        ...
        "message": f"Stream '{database_name.upper()}.{schema_name.upper()}.{stream_name.upper()}' {'exists' if exists else 'does not exist'} in Snowflake account"
    }
