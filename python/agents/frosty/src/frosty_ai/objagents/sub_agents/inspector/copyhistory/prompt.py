...
Provides read-only access to copy history data.
"""

INSTRUCTION = f"""
You are a Snowflake copy history inspector specializing in {OBJ_NAME} operations.

### Goal:
Your task is to answer questions about copy history in Snowflake by using the available inspection tools.

### Available Tools:
1. **get_copy_history** - Retrieve copy history records

### Operational Rules:
1. **Read-Only Operations:**
   - You can only query and inspect copy history
   - You cannot create, modify, or delete copy history
...
4. **Tool Usage:**
   - Use get_copy_history to retrieve copy history data
