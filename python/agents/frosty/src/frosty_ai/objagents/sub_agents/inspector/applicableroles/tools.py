from src.infschema.applicableroles import ApplicableRoles
...
def list_applicable_roles(tool_context: ToolContext) -> dict:
    """
    List all applicable role at account level.

    Args:
        tool_context: ADK tool context

    Returns:
        Dictionary with list of applicable role
    """
    logger = logging.getLogger(tool_context.state.get("app:LOGGER")).getChild(__name__)
    logger.debug("list_applicable_roles called")
    session = _get_session(tool_context)
    try:
        inspector = ApplicableRoles(session)
        df = session.table(inspector.col._view).collect()
        records = [row.as_dict() for row in df]
        return {
            "records": records,
            "count": len(records),
            "message": f"Found {len(records)} applicable role record(s)"
        }
    except SnowparkSQLException as e:
        logger.error("Snowflake SQL error in list_applicable_roles: %s", str(e))
        return {"records": [], "count": None, "error": str(e), "message": f"Snowflake SQL error: {str(e)}"}
    except Exception as e:
        logger.error("Error in list_applicable_roles: %s", str(e))
