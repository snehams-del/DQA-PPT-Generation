from src.infschema.icebergtablesnapshotrefreshhistory import IcebergTableSnapshotRefreshHistory
...
def get_iceberg_table_snapshot_refresh_history(database_name: str, tool_context: ToolContext, result_limit: int = 100) -> dict:
    """
    Get iceberg table snapshot refresh history data.
    ...
    Returns:
        Dictionary with iceberg table snapshot refresh history records
    """
    logger = logging.getLogger(tool_context.state.get("app:LOGGER")).getChild(__name__)
    logger.debug("get_iceberg_table_snapshot_refresh_history called for database '%s'", database_name)
    session = _get_session(tool_context)
    try:
        inspector = IcebergTableSnapshotRefreshHistory(session)
        ...
        return {
            ...
            "records": records,
            "count": len(records),
            "message": f"Found {len(records)} iceberg table snapshot refresh history record(s)"
        }
    except SnowparkSQLException as e:
        logger.error("Snowflake SQL error in get_iceberg_table_snapshot_refresh_history: %s", str(e))
        return {"records": [], "count": None, "error": str(e), "message": f"Snowflake SQL error: {str(e)}"}
    except Exception as e:
        logger.error("Error in get_iceberg_table_snapshot_refresh_history: %s", str(e))
