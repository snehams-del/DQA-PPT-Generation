from src.infschema.warehousemeteringhistory import WarehouseMeteringHistory
...
def get_warehouse_metering_history(database_name: str, tool_context: ToolContext, warehouse_name: str = None, result_limit: int = 100) -> dict:
    """
    Get warehouse metering history data.
    ...
    Returns:
        Dictionary with warehouse metering history records
    """
    logger = logging.getLogger(tool_context.state.get("app:LOGGER")).getChild(__name__)
    logger.debug("get_warehouse_metering_history called for database '%s'", database_name)
    ...
    try:
        ...
        return {
            ...
            "records": records,
            "count": len(records),
            "message": f"Found {len(records)} warehouse metering history record(s)"
        }
    except SnowparkSQLException as e:
        logger.error("Snowflake SQL error in get_warehouse_metering_history: %s", str(e))
        return {"records": [], "count": None, "error": str(e), "message": f"Snowflake SQL error: {str(e)}"}
    except Exception as e:
        logger.error("Error in get_warehouse_metering_history: %s", str(e))
