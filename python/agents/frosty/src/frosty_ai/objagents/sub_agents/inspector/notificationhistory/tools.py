from src.infschema.notificationhistory import NotificationHistory
...
def get_notification_history(database_name: str, tool_context: ToolContext, result_limit: int = 100) -> dict:
    """
    ...
    Returns:
        Dictionary with notification history records
    """
    logger = logging.getLogger(tool_context.state.get("app:LOGGER")).getChild(__name__)
    logger.debug("get_notification_history called for database '%s'", database_name)
    session = _get_session(tool_context)
    try:
        inspector = NotificationHistory(session)
        ...
        return {
            ...
            "records": records,
            "count": len(records),
            "message": f"Found {len(records)} notification history record(s)"
        }
    except SnowparkSQLException as e:
        logger.error("Snowflake SQL error in get_notification_history: %s", str(e))
        return {"records": [], "count": None, "error": str(e), "message": f"Snowflake SQL error: {str(e)}"}
    except Exception as e:
        logger.error("Error in get_notification_history: %s", str(e))
