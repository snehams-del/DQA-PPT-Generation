...
@patch("presentation_agent.shared_libraries.model_armor.MODEL_ARMOR_TEMPLATE_ID", "test-template")
@patch("presentation_agent.shared_libraries.model_armor.google.auth.default")
@patch("httpx.AsyncClient.post")
@pytest.mark.asyncio
async def test_model_armor_interceptor_allow(mock_post, mock_auth, mock_callback_context):
...
@patch("httpx.AsyncClient.post")
@pytest.mark.asyncio
async def test_model_armor_interceptor_block(mock_post, mock_auth, mock_callback_context):
    mock_auth.return_value = (MagicMock(), "test-project")
    mock_post.return_value = create_mock_response({
        "sanitizationResult": {
...
@patch("httpx.AsyncClient.post")
@pytest.mark.asyncio
async def test_model_armor_response_interceptor_allow(mock_post, mock_auth, mock_callback_context, mock_llm_response):
    mock_auth.return_value = (MagicMock(), "test-project")
    mock_post.return_value = create_mock_response({
        "sanitizationResult": {
            "sanitizationVerdict": "MODEL_ARMOR_SANITIZATION_VERDICT_ALLOW"
    ...
    result = await model_armor_response_interceptor(mock_callback_context, mock_llm_response)
    assert result is None
...
@patch("httpx.AsyncClient.post")
@pytest.mark.asyncio
async def test_model_armor_response_interceptor_redact(mock_post, mock_auth, mock_callback_context, mock_llm_response):
    mock_auth.return_value = (MagicMock(), "test-project")
    mock_post.return_value = create_mock_response({
        "sanitizationResult": {
            "sanitizationVerdict": "MODEL_ARMOR_SANITIZATION_VERDICT_ALLOW",
...
@pytest.mark.asyncio
async def test_model_armor_interceptor_no_project(mock_getenv, mock_auth, mock_callback_context):
    mock_auth.return_value = (MagicMock(), None)
    # Make getenv return None for GOOGLE_CLOUD_PROJECT
    mock_getenv.side_effect = lambda k, d=None: d if k != "GOOGLE_CLOUD_PROJECT" else None
...
@patch("presentation_agent.shared_libraries.model_armor.google.auth.default")
@pytest.mark.asyncio
async def test_model_armor_interceptor_exception(mock_auth, mock_callback_context):
...
@patch("httpx.AsyncClient.post")
@pytest.mark.asyncio
async def test_model_armor_interceptor_fail_closed_memory(mock_post, mock_auth, mock_callback_context, monkeypatch):
...
@patch("presentation_agent.shared_libraries.model_armor.MODEL_ARMOR_TEMPLATE_ID", None)
@pytest.mark.asyncio
...
@patch("presentation_agent.shared_libraries.model_armor.MODEL_ARMOR_TEMPLATE_ID", None)
@pytest.mark.asyncio
async def test_model_armor_response_interceptor_no_template_id(mock_callback_context, mock_llm_response):
    result = await model_armor_response_interceptor(mock_callback_context, mock_llm_response)
    assert result is None

@patch("presentation_agent.shared_libraries.model_armor.MODEL_ARMOR_TEMPLATE_ID", "test-template")
@pytest.mark.asyncio
async def test_model_armor_response_interceptor_no_llm_content(mock_callback_context):
    resp = MagicMock(spec=LlmResponse)
    resp.content = None
...
@patch("presentation_agent.shared_libraries.model_armor.MODEL_ARMOR_TEMPLATE_ID", "test-template")
@pytest.mark.asyncio
async def test_model_armor_response_interceptor_no_llm_text(mock_callback_context):
    resp = MagicMock(spec=LlmResponse)
...
@patch("presentation_agent.shared_libraries.model_armor.google.auth.default")
@pytest.mark.asyncio
async def test_model_armor_response_interceptor_fail_closed(mock_auth, mock_callback_context, mock_llm_response, monkeypatch):
    mock_auth.side_effect = Exception("Auth failed")
...
@patch("presentation_agent.shared_libraries.model_armor.google.auth.default")
@pytest.mark.asyncio
async def test_model_armor_response_interceptor_fail_closed_memory(mock_auth, mock_callback_context, mock_llm_response, monkeypatch):
    mock_auth.side_effect = Exception("Auth failed")
    monkeypatch.setenv("USE_IN_MEMORY_FOR_TESTS", "true")
...
@patch("httpx.AsyncClient.post")
@pytest.mark.asyncio
async def test_model_armor_response_interceptor_block(mock_post, mock_auth, mock_callback_context, mock_llm_response):
    mock_auth.return_value = (MagicMock(), "test-project")
    mock_post.return_value = create_mock_response({
        "sanitizationResult": {
            "sanitizationVerdict": "MODEL_ARMOR_SANITIZATION_VERDICT_BLOCK",
    ...
    result = await model_armor_response_interceptor(mock_callback_context, mock_llm_response)
    assert result is not None
    assert "blocked by enterprise security policy" in result.content.parts[0].text
