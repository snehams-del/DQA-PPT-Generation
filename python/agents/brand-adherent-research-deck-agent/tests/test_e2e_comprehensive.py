...
async def test_comprehensive_e2e_flow():
    ...
    outline_json = {
        "outline": {
            ...
            "slides": [{"title": "Market Trends", "layout_name": "Title and Content", "bullets": ["Focus on growth"]}],
            "closing_title": "End"
        }
    }
    ...
    slide_content_json = {
        ...
        "citations": ["https://market-data.com/2026", "https://science-hub.org/ai"]
    }
    ...
    with patch("presentation_agent.tools.presentation_orchestrator.get_gcs_file_as_local_path", AsyncMock(return_value="mock.pptx")), \
         ...
         patch("presentation_agent.tools.presentation_orchestrator.render_deck_from_spec", AsyncMock(return_value="final_output.pptx")), \
         patch("presentation_agent.tools.presentation_orchestrator.save_presentation", side_effect=mock_save_pres), \
    ...
    assert pptx_found
    print(f"✅ PASS: Final presentation VISIBLE in sidebar.")
