...
async def test_full_citation_workflow():
    ...
    mock_outline = PresentationOutline(
        ...
        slides=[
            SlideSpec(
                ...
                citations=["https://www.grandviewresearch.com/industry-analysis/renewable-energy-market"]
            )
        ],
        closing_title="Thank You"
    )
    
    mock_outliner_response = MockResponse(json.dumps({"outline": mock_outline.model_dump()}))
    
    with patch("presentation_agent.sub_agents.synthesizer.agent.initialize_genai_client") as mock_init:
        mock_client = mock_init.return_value
        mock_client.models.generate_content.return_value = mock_outliner_response
    ...
    assert result["slides"][0]["citations"] == ["https://www.grandviewresearch.com/industry-analysis/renewable-energy-market"]
    print("[Step 2] Outline generation verified: Citations extracted from summary into SlideSpec.")

    # --- STEP 3: Slide Generation Verification ---
    mock_written_slide = SlideSpec(
        title="Market Overview",
        bullets=["The global renewable energy market hit $2.15 trillion in 2023."],
        layout_name="Title and Content",
        citations=["https://www.grandviewresearch.com/industry-analysis/renewable-energy-market"],
        speaker_notes="Market growth is substantial."
    )
    
    mock_writer_response = MockResponse(json.dumps(mock_written_slide.model_dump()))
    
    with patch("presentation_agent.sub_agents.synthesizer.agent.initialize_genai_client") as mock_init:
        mock_client = mock_init.return_value
        mock_client.aio.models.generate_content = AsyncMock(return_value=mock_writer_response)
    ...
    assert final_slide["citations"] == ["https://www.grandviewresearch.com/industry-analysis/renewable-energy-market"]
    print("[Step 3] Slide generation verified: Citations preserved in final DeckSpec.")

    # --- STEP 4: Render Verification ---
    deck_spec_dict = result["deck_spec"]
    
    with patch("presentation_agent.tools.presentation_orchestrator.Presentation") as mock_prs_class:
        ...
        mock_prs.slides.__getitem__.side_effect = lambda i: [mock_cover_slide][i]
        mock_prs.slides.__iter__.side_effect = lambda: iter([mock_cover_slide])
        mock_prs.slides.add_slide = MagicMock(side_effect=[mock_body_slide, mock_closing_slide])
        ...
        with patch("os.path.exists", return_value=True):
             with patch("presentation_agent.tools.presentation_orchestrator.get_smart_layout"):
                 with patch("presentation_agent.tools.presentation_orchestrator._insert_image"):
                    with patch("tempfile.NamedTemporaryFile") as mock_temp:
                        mock_temp.return_value.__enter__.return_value.name = "dummy.pptx"
        ...
        assert "https://www.grandviewresearch.com/industry-analysis/renewable-energy-market" in assigned_notes
        print("[Step 4] Render verified: Citations appended to speaker notes in PPTX.")
