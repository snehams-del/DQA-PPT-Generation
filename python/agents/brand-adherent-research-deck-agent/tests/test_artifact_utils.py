...
from presentation_agent.shared_libraries.config import PRESENTATION_SPEC_ARTIFACT
...
async def test_update_slide_in_spec_success(mock_context):
    ...
    assert updated_spec["slides"][1]["layout_name"] == "Title and Image" # Changed
...
async def test_update_slide_in_spec_append_growth(mock_context):
    ...
    assert updated_spec["slides"][2]["title"] == "New Slide 3" # Auto-filled gap
