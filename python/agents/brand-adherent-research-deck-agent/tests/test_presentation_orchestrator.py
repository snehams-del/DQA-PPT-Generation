...
async def test_render_deck_from_spec(mock_generate_visual):
    ...
    try:
        ...
        with patch("presentation_agent.tools.presentation_orchestrator._insert_visual_into_slide") as mock_insert:
        ...
        if 'result' in locals() and os.path.exists(result) and result != output_path:
            os.remove(result)
