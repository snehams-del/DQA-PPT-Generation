...
from presentation_agent.sub_agents.synthesizer.agent import generate_and_save_outline, batch_generate_slides
from presentation_agent.tools.presentation_orchestrator import generate_and_render_deck
from presentation_agent.shared_libraries.config import RESEARCH_SUMMARY_ARTIFACT, PRESENTATION_SPEC_ARTIFACT
...
async def test_e2e_state_first_workflow():
    ...
    tool_context.load_artifact = AsyncMock(return_value=None) # Ensure no artifact fallback is used
    ...
    outline_json = {
        "outline": {
            ...
            "slides": [{"title": "Slide 1", "layout_name": "Title and Content", "bullets": ["Focus 1"]}],
            "closing_title": "End"
        }
    }
    mock_client.aio.models.generate_content.return_value = MockResponse(json.dumps(outline_json))
    ...
    assert tool_context.state["research_summary"] == "Test Research [https://test.com]"
    ...
    mock_client.aio.models.generate_content.return_value = MockResponse(json.dumps(slide_content_json))
    ...
    assert tool_context.state["current_deck_spec"]["slides"][0]["bullets"] == ["Detail 1", "Detail 2"]
    ...
    with patch("presentation_agent.tools.presentation_orchestrator.get_gcs_file_as_local_path", AsyncMock(return_value="mock.pptx")), \
         patch("presentation_agent.tools.presentation_orchestrator.Presentation", MagicMock()), \
         patch("presentation_agent.tools.presentation_orchestrator.render_deck_from_spec", AsyncMock(return_value="final.pptx")), \
         patch("presentation_agent.tools.presentation_orchestrator.save_presentation", AsyncMock(return_value="Success")), \
         patch("presentation_agent.tools.presentation_orchestrator.os.remove", MagicMock()):
        
        # NO arguments passed - must use state!
        render_result = await generate_and_render_deck(tool_context=tool_context)
    ...
    artifact_names = [call.args[0] for call in tool_context.save_artifact.call_args_list]
    ...
    print("✅ UI Sidebar Cleanliness: Internal files were NOT registered as artifacts.")
