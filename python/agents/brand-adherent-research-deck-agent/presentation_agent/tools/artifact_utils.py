...
async def get_artifact_as_local_path(
    ...
    try:
        ...
        if not file_artifact:
            log.warning(f"Artifact '{artifact_name}' not found on first try. Waiting 2 seconds for cloud sync...")
...
async def get_gcs_file_as_local_path(gcs_uri: str = DEFAULT_TEMPLATE_URI) -> str:
    ...
    try:
        ...
        if not blob.exists():
            return (
                f"Error: The file does not exist at the specified GCS path: {gcs_uri}"
            )
        ...
        log.error(
            f"Failed to download default template from '{gcs_uri}': {e}", exc_info=True
        )
...
async def save_presentation(
    ...
    try:
        ...
        log.info(f"Successfully saved presentation as artifact '{new_artifact_name}'.")
...
async def save_deck_spec(tool_context: ToolContext, deck_spec: dict) -> str:
    ...
    try:
        ...
        return f"Error: Internal failure while saving plan."
    ...
    try:
        ...
        if 0 <= slide_index < len(slides):
            ...
            while len(slides) < slide_index:
                slides.append({"title": f"New Slide {len(slides)+1}", "bullets": [], "layout_name": "Title and Content"})
            
            new_slide = {"title": "New Slide", "bullets": [], "layout_name": "Title and Content"}
        ...
        return f"Error: Failed to update slide copy."
