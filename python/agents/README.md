...
| [Sequential Workflow Automation](sequential-workflow-automation) | The "Sequential Workflow Automation" pattern is for agents that automate a complex, multi-step business process. Multiple distinct tasks or transactions must be executed in a predetermined, fixed sequence across various systems to complete a full workflow | Multi-agent, Custom tool, BigQuery, Agent Tool | Workflow | Advanced | Multi Agent / Sequential Agent | Order Management / Customer Support |
