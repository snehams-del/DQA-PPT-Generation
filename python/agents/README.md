...
| [Capital Guess Game](capital-guess-game) | An interactive game where the agent attempts to guess the capital of a country chosen by the user. Demonstrates stateful multi-agent workflows and custom agent orchestration using Human-In-The-Loop example. | Multi-agent, State management, Custom tools, Gaming | Conversational | Easy | Multi Agent | Gaming / Entertainment |
