...
def triageQueryTool(hostname: str, alert_type: str) -> str:
    ...
    job_config = bigquery.QueryJobConfig(
        query_parameters=[
            ...
            bigquery.ScalarQueryParameter("alert_type", "STRING", alert_type), # Updated parameter
        ]
    )
    duplicate_rows = list(client.query(dedup_query, job_config=job_config).result())

    if duplicate_rows:
        return json.dumps({"is_duplicate": True, "existing_incident": duplicate_rows[0]['IncidentID']})
...
def threatIntelQueryTool(indicators: list) -> str:
    ...
    """
    job_config = bigquery.QueryJobConfig(
        query_parameters=[bigquery.ArrayQueryParameter("indicators", "STRING", indicators)]
    )
    rows = list(client.query(query, job_config=job_config).result())
...
def getPlaybookTool(triggering_condition: str) -> str:
    ...
    """
    job_config = bigquery.QueryJobConfig(
        query_parameters=[bigquery.ScalarQueryParameter("trigger", "STRING", triggering_condition)]
    )
    rows = list(client.query(query, job_config=job_config).result())
    return bq_rows_to_json(rows)
...
def createIncidentTool(alert_type: str, hostname: str, user: str, severity: str) -> str:
    ...
    job_config = bigquery.QueryJobConfig(
        query_parameters=[
            ...
            bigquery.ScalarQueryParameter("creation_timestamp", "TIMESTAMP", creation_timestamp),
