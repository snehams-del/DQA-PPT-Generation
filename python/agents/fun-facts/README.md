...
NOTE: For this agent, `--region` must be passed explicitly because the Gemini model requires the `global` endpoint, but Agent Engine must be deployed to a regional endpoint (e.g. ``).

```bash
adk deploy agent_engine fun_facts --region=""
```
...
```none
projects/PROJECT_NUMBER/locations//reasoningEngines/AGENT_ENGINE_ID
```
