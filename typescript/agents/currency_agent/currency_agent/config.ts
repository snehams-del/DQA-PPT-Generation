...
/**
 * Configuration module for the currency agent.
 */
...
export class AgentModel {
  name: string = 'currency_agent';
  model: string = 'gemini-2.5-flash';
}

export class Config {
  agentSettings: AgentModel = new AgentModel();
  appName: string = 'currency_agent_app';
  ...
  GENAI_USE_VERTEXAI: string = getEnv('GOOGLE_GENAI_USE_VERTEXAI', 'FALSE');
  API_KEY: string = getEnv('GOOGLE_API_KEY', '');
  MCP_SERVER_URL: string = getEnv('MCP_SERVER_URL', 'http://localhost:8080/mcp');
}
